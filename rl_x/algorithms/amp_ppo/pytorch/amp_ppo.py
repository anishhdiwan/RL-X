...
from rl_x.algorithms.amp_ppo.pytorch.policy import Policy
from rl_x.algorithms.amp_ppo.pytorch.critic import Critic
from rl_x.algorithms.amp_ppo.pytorch.data_utils import prepare_expert_data
from rl_x.algorithms.amp_ppo.pytorch.discriminator import Discriminator
...
class AMP_PPO:
    ...
    def discriminator_loss(self, states, actions, next_states, absorbing, expert_states, expert_actions, expert_next_states, expert_absorbing, log_probs, expert_log_probs):
        ...
        agent_loss = (logits + 1).square().mean()
        expert_loss = (expert_logits - 1).square().mean()
    ...
    def update(self, batch, learning_iteration_step):
        ...
        with torch.no_grad():
            ...
            corrected_reward = (1 - 0.25 * (logits - 1).square()).clamp(min=0)
            absorbing_reward = (1 - 0.25 * (absorbing_logits - 1).square()).clamp(min=0)
        with torch.no_grad():
