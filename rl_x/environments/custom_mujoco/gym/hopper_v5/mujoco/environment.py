...
from rl_x.environments.custom_mujoco.gym.hopper_v5.mujoco.viewer import MujocoViewer
...
class Hopper(gym.Env):
    ...
    def __init__(self, render, horizon=1000):
        ...
        initial_qpos = [0.0, 1.25] + [0.0] * (self.mj_model.nq - 2)
    ...
    def get_reward(self, data, x_position_before):
        ...
        state = np.concatenate([data.qpos, data.qvel])[2:]
        healthy_state = np.all((state > min_state) & (state < max_state))
        is_healthy = np.clip(
            np.nan_to_num((healthy_state & (torso_height > min_z) & (torso_height < max_z) & (torso_pitch > min_angle) & (torso_pitch < max_angle)).astype("float32")),
            a_min=0.0,
            a_max=1.0,
        )
