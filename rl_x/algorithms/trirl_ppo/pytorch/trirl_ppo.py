...
from rl_x.algorithms.trirl_ppo.pytorch.policy import Policy
from rl_x.algorithms.trirl_ppo.pytorch.critic import Critic
from rl_x.algorithms.trirl_ppo.pytorch.data_utils import prepare_expert_data
from rl_x.algorithms.trirl_ppo.pytorch.discriminator import Discriminator
from rl_x.algorithms.trirl_ppo.pytorch.reward_function import RewardFunction
...
class TRIRL_PPO:
    ...
    def update(self, batch, learning_iteration_step):
        ...
        self.etas_buffer.append(all_etas[-1][0] * torch.ones(shape, device=self.device))
    ...
    def policy_loss(self, states, actions, old_log_probs, advantages, old_means, old_logstds, global_step):
        ...
        eta = cfg.init_eta if cfg.const_eta else cfg.init_eta * (1 - min(global_step, cfg.total_timesteps) / cfg.total_timesteps)
        old_std = old_logstds[-1:].exp()
        trust_region_loss = 0.5 * (((old_means - mean) / old_std).square() + 2 * (old_logstds[-1:] - logstd) + (logstd.exp() / old_std).square() - 1).sum(-1)
        etas = torch.full((states.shape[0],), eta, device=self.device)
        ...
        loss = policy_loss - cfg.entropy_coef * entropy.mean() + eta * trust_region_loss.mean()
