...
from rl_x.algorithms.amp_ppo.flax.policy import get_policy
from rl_x.algorithms.amp_ppo.flax.critic import get_critic
from rl_x.algorithms.amp_ppo.flax.discriminator import get_discriminator
from rl_x.algorithms.amp_ppo.flax.data_utils import prepare_expert_data
...
class AMP_PPO:
    ...
    def update(self, train_state, batch, expert_data, key, learning_iteration_step):
        policy_state, critic_state, discriminator_state = train_state
        ...
        def amp_loss_fn(discriminator_params, state, action, expert_state, expert_action, label, expert_label, next_state=None, absorbing=None, expert_next_state=None, expert_absorbing=None):
            ...
            mse_agent = optax.squared_error(logits, label).mean()
            mse_expert = optax.squared_error(expert_logits, expert_label).mean()
            ...
            metrics = {
                "loss/discriminator_loss": mse_loss,
                "loss/discriminator_agent_loss": mse_agent,
                "loss/discriminator_expert_loss": mse_expert,
        ...
        mean_vmapped_amp_loss_fn = lambda *a, **k: tree.map_structure(safe_mean, vmap_amp_loss_fn(*a, **k))
        grad_amp_loss_fn = jax.value_and_grad(mean_vmapped_amp_loss_fn, argnums=(0), has_aux=True)
        ...
        def amp_minibatch_update(carry, minibatch_indices_disc):
            ...
            (amp_loss, (metrics)), (discriminator_gradients) = grad_amp_loss_fn(
        ...
        carry, (disc_optimization_metrics) = jax.lax.scan(amp_minibatch_update, init_carry, batch_indices_disc)
        discriminator_state = carry
        ...
        def get_reward(inputs, discriminator_state):
            ...
            reward = jnp.maximum(0, 1 - 0.25*(logits - 1)**2)
            return reward

        get_reward = jax.vmap(get_reward, in_axes=(0, None), out_axes=0)

        amp_reward = get_reward((states.reshape((-1,) + self.os_shape),
        ...
        if self.handle_absorbing_states:
            amp_reward_absorbing_state = get_reward((next_states.reshape((-1,) + self.os_shape),
                        actions.reshape((-1,) + self.as_shape),
                        next_states.reshape((-1,) + self.os_shape),
        ...
        if self.handle_absorbing_states:
            advantages, returns = calculate_gae_advantages_absorbing(critic_state, next_states, amp_reward, amp_reward_absorbing_state, values, terminations)
        else:
            advantages, returns = calculate_gae_advantages(critic_state, next_states, amp_reward, values, terminations)
        ...
        return (policy_state, critic_state, discriminator_state), combined_metrics, key
    ...
    def train(self):
        ...
        for learning_iteration_step in range(int(self.nr_updates)):
            ...
            train_state, metrics, self.key = self.update((self.policy_state, self.critic_state, self.discriminator_state), batch, self.expert_data, self.key, learning_iteration_step)
            self.policy_state, self.critic_state, self.discriminator_state = train_state
            ...
            if global_step % self.evaluation_and_save_frequency == 0:
                if self.evaluation_active:
                    ...
                    if self.eval_env is self.train_env:
                        observation, _ = self.train_env.reset()
                if self.save_model:
                    self.save(self.policy_state, self.critic_state, self.discriminator_state)
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        model = AMP_PPO(config, train_env, eval_env, run_path, writer)
        if config.runner.mode == "test":
