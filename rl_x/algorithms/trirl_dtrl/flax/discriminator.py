...
class RewardApproximator(nn.Module):
    ...
    def __call__(self, x, y, xn, shaping=None):
        ...
        """
        x = jnp.concatenate([x.flatten(), y.flatten()])
        reward = nn.Dense(512, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0))(x)
        reward = nn.relu(reward)
        reward = nn.Dense(256, kernel_init=orthogonal(np.sqrt(2)), bias_init=constant(0.0))(reward)
        reward = nn.relu(reward)
