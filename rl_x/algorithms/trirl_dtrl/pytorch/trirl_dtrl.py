...
from rl_x.algorithms.trirl_dtrl.pytorch.policy import Policy
from rl_x.algorithms.trirl_dtrl.pytorch.critic import Critic
from rl_x.algorithms.trirl_dtrl.pytorch.data_utils import prepare_expert_data
from rl_x.algorithms.trirl_dtrl.pytorch.discriminator import Discriminator
from rl_x.algorithms.trirl_dtrl.pytorch.reward_function import RewardFunction
from rl_x.algorithms.trirl_dtrl.pytorch.trust_region_layer import kl_projection, entropy_projection
...
class TRIRL_DTRL:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.reward_fn = RewardFunction(config, train_env).to(self.device)
        self.networks["reward_fn"] = self.reward_fn
        self.disc_buffer = []
        self.etas_buffer = []
        self.policy_buffer = []
        if config.algorithm.on_demand_etas:
            self.policy_buffer.append({key: value.detach().clone() for key, value in self.policy.state_dict().items()})
        self.optimizers = {}
    ...
    @torch.no_grad()
    def correct_reward(self, states, actions, next_states, absorbing, etas):
        cfg = self.config.algorithm
        correction = torch.zeros(states.shape[0], device=self.device)
        for start in range(0, len(self.disc_buffer), cfg.chunk_size):
            chunk = self.disc_buffer[start:start + cfg.chunk_size]
            stacked = {key: torch.stack([entry[key] for entry in chunk]) for key in chunk[0]}
            outputs = vmap(lambda parameters: functional_call(self.discriminator, parameters, (states, actions, next_states, absorbing)))(stacked)
            for offset, output in enumerate(outputs):
                eta = etas[start + offset]
                eta = eta.flatten() if cfg.get("on_demand_etas", False) else eta.max()
                step = cfg.epsilon / (1 + eta.clamp(min=0))
                correction = (1 - step) * correction + step / cfg.entropy_coef * output
        return cfg.entropy_coef * correction


    @torch.no_grad()
    def compute_etas(self, states, shape):
        cfg = self.config.algorithm
        etas = []
        for start in range(0, max(len(self.policy_buffer) - 1, 0), cfg.chunk_size - 1):
            chunk = self.policy_buffer[start:start + cfg.chunk_size]
            stacked = {key: torch.stack([entry[key] for entry in chunk]) for key in chunk[0]}
            means, logstds = vmap(lambda parameters: functional_call(self.policy, parameters, (states,)))(stacked)
            for index in range(len(chunk) - 1):
                current_std = logstds[index + 1].exp().detach()
                old_std = logstds[index].exp().detach()
                maha = 0.5 * ((means[index + 1] - means[index]) / old_std).square().sum(-1)
                eta_mu = (torch.sqrt(maha / cfg.mean_bound) - 1).clamp(min=0)

                with torch.enable_grad():
                    eta = torch.zeros(1, device=self.device, requires_grad=True)
                    optimizer = torch.optim.LBFGS([eta], max_iter=current_std.shape[-1], max_eval=10000, tolerance_grad=1e-9, tolerance_change=0, history_size=10, line_search_fn="strong_wolfe")  # Match the reference's action-dimension iteration budget.

                    def objective():
                        optimizer.zero_grad()
                        positive_eta = eta.clamp(min=0)
                        std = ((positive_eta + 1) / (positive_eta / old_std.square() + current_std.reciprocal().square()).clamp(min=1e-8)).sqrt().clamp(min=1e-8)
                        value = positive_eta * cfg.cov_bound - positive_eta * old_std.log().sum() + (positive_eta + 1) * std.log().sum()
                        value.backward()
                        return value

                    optimizer.step(objective)
                etas.append(torch.maximum(eta_mu, eta.detach().squeeze()).reshape(shape))
        return etas + [torch.zeros(shape, device=self.device)]
    ...
    def update(self, batch, learning_iteration_step):
        ...
        self.disc_buffer.append({key: value.detach().clone() for key, value in self.discriminator.state_dict().items()})
        self.disc_buffer = self.disc_buffer[-cfg.disc_buffer_capacity:]
        etas = list(self.etas_buffer) + [torch.zeros(shape, device=self.device)]
        if cfg.on_demand_etas:
            etas = self.compute_etas(states, shape)
        corrected_reward = self.correct_reward(states, actions, next_states, terminations, etas)
        absorbing_reward = self.correct_reward(next_states, actions, next_states, torch.ones_like(terminations), etas)
        if cfg.reward_fn_approximator:
            reward_losses = []
            for indices in self.minibatches(self.batch_size, cfg.minibatch_size, cfg.nr_epochs_rew):
                loss = (self.reward_fn(states[indices], actions[indices]) - corrected_reward[indices]).square().mean()
                self.optimize("reward_fn", loss)
                reward_losses.append(loss.detach())
            metrics["loss/reward_fn_loss"] = torch.stack(reward_losses).mean()
            with torch.no_grad():
                corrected_reward = self.reward_fn(states, actions)
                absorbing_reward = self.reward_fn(next_states, actions)
        with torch.no_grad():
        ...
        self.etas_buffer.append(torch.cat(all_etas).reshape(cfg.nr_epochs, *shape).amax(0))
        self.etas_buffer = self.etas_buffer[-(cfg.disc_buffer_capacity - 1):] if cfg.disc_buffer_capacity > 1 else []
        if cfg.on_demand_etas:
            self.policy_buffer.append({key: value.detach().clone() for key, value in self.policy.state_dict().items()})
            self.policy_buffer = self.policy_buffer[-cfg.disc_buffer_capacity:]
        return metrics
    ...
    def policy_loss(self, states, actions, old_log_probs, advantages, old_means, old_logstds, global_step):
        ...
        old_logstd = old_logstds[-1:]
        old_entropy = (old_logstd + 0.5 * math.log(2 * math.pi * math.e)).sum()
        beta = old_entropy * 0.5 ** (10 * min(global_step, cfg.total_timesteps) / cfg.total_timesteps)
        projected_mean, projected_std, eta_mu, eta_cov, kl_mean, post_kl_mean, kl_cov, post_kl_cov = kl_projection(mean, logstd.exp(), old_means, old_logstd.exp(), cfg.mean_bound, cfg.cov_bound)
        trust_region_loss = 0.5 * (((projected_mean.detach() - mean) / projected_std.detach()).square() + 2 * (projected_std.detach().log() - logstd) + (logstd.exp() / projected_std.detach()).square() - 1).sum(-1)
        mean = projected_mean
        logstd = entropy_projection(projected_std.log(), beta, mean.shape[-1])
        etas = torch.maximum(eta_mu, eta_cov)
        new_log_probs = (-0.5 * ((actions - mean) / logstd.exp()).square() - 0.5 * math.log(2 * math.pi) - logstd).sum(-1)
        entropy = (logstd + 0.5 * math.log(2 * math.pi * math.e)).sum(-1) * cfg.entropy_coef
        ...
        loss = policy_loss - cfg.entropy_coef * entropy.mean() + cfg.trust_region_coef * trust_region_loss.mean()
        ...
        metrics["loss/trust_region_loss"] = trust_region_loss.detach().mean()
        metrics.update({"projection/eta_mu": eta_mu.detach().mean(), "projection/eta_cov": eta_cov.detach(), "projection/unprojected_kl_mean": kl_mean.detach().mean(), "projection/projected_kl_mean": post_kl_mean.detach().mean(), "projection/unprojected_kl_cov": kl_cov.detach(), "projection/projected_kl_cov": post_kl_cov.detach()})
    ...
    def save(self):
        ...
        if not self.config.algorithm.reward_fn_approximator:
            checkpoint["disc_buffer"] = self.disc_buffer
            checkpoint["etas_buffer"] = self.etas_buffer
            checkpoint["policy_buffer"] = self.policy_buffer
    ...
    def load(config, train_env, eval_env, run_path, writer, explicitly_set_algorithm_params):
        ...
        if "disc_buffer" in checkpoint:
            for name in ["disc_buffer", "etas_buffer", "policy_buffer"]:
                entries = checkpoint[name]
                setattr(model, name, [{key: value.to(model.device) for key, value in entry.items()} if isinstance(entry, dict) else entry.to(model.device) for entry in entries])
        return model
