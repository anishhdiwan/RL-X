...
from rl_x.algorithms.trirl_dtrl.flax.policy import get_policy
from rl_x.algorithms.trirl_dtrl.flax.critic import get_critic
from rl_x.algorithms.trirl_dtrl.flax.buffer import ParamsBuffer, EtasBuffer
from rl_x.algorithms.trirl_dtrl.flax.discriminator import get_discriminator, get_reward_approximator
from rl_x.algorithms.trirl_dtrl.flax.data_utils import prepare_expert_data
from rl_x.algorithms.trirl_dtrl.flax.reward_correction import make_chunked_ensemble_rew_correct
from rl_x.algorithms.trirl_dtrl.flax.trust_region_layer import make_chunked_compute_etas, kl_cov_proj, kl_projection, entropy_projection
...
class TRIRL_DTRL:
    ...
    def update(self, train_state, batch, expert_data, key, learning_iteration_step):
        policy_state, critic_state, discriminator_state, reward_fn_state, disc_buffer, etas_buffer, policy_buffer = train_state
        ...
        def plcy_apply_fn(inputs, policy_state):
            state = inputs
        ...
        plcy_apply_fn = jax.vmap(plcy_apply_fn, in_axes=(0, None), out_axes=(0, 0))
        ...
        return (policy_state, critic_state, discriminator_state, reward_fn_state, disc_buffer, etas_buffer, policy_buffer), combined_metrics, key
    ...
    def train(self):
        ...
        for learning_iteration_step in range(int(self.nr_updates)):
            ...
            train_state, metrics, self.key = self.update((self.policy_state, self.critic_state, self.discriminator_state, self.reward_fn_state, self.disc_buffer, self.etas_buffer, self.policy_buffer), batch, self.expert_data, self.key, learning_iteration_step)
            self.policy_state, self.critic_state, self.discriminator_state, self.reward_fn_state, self.disc_buffer, self.etas_buffer, self.policy_buffer = train_state
            ...
            if global_step % self.evaluation_and_save_frequency == 0:
                if self.evaluation_active:
                    ...
                    if self.eval_env is self.train_env:
                        observation, _ = self.train_env.reset()
                if self.save_model:
                    self.save(self.policy_state, self.critic_state, self.discriminator_state, self.reward_fn_state, self.disc_buffer, self.etas_buffer, self.policy_buffer)
