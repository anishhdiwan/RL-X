...
from rl_x.algorithms.near_ppo.pytorch.policy import Policy
from rl_x.algorithms.near_ppo.pytorch.critic import Critic
from rl_x.algorithms.near_ppo.pytorch.data_utils import prepare_expert_data
from rl_x.algorithms.near_ppo.pytorch.energy_function import EnergyFunction
...
class NEAR_PPO:
    def __init__(self, config, train_env, eval_env, run_path, writer):
        ...
        self.energyfn = EnergyFunction(config, train_env).to(self.device)
        self.networks["energyfn"] = self.energyfn
        self.optimizers = {}
    ...
    def energy_loss(self, samples, conditions, noise):
        cfg = self.config.algorithm
        sigmas = torch.exp(torch.linspace(math.log(cfg.sigma_begin_ncsn), math.log(cfg.sigma_end_ncsn), cfg.L_ncsn, device=self.device))
        sigma = sigmas[conditions]
        perturbed = (samples + noise * sigma[:, None]).detach().requires_grad_(True)
        target = -(perturbed.detach() - samples) / sigma[:, None].square()
        energy = self.energyfn(perturbed, conditions if cfg.ncsnv1 else sigma)
        score = torch.autograd.grad(energy.sum(), perturbed, create_graph=True)[0]
        return (0.5 * (score - target).square().sum(-1) * sigma.pow(cfg.anneal_power_ncsn)).mean()


    def pretrain(self):
        cfg = self.config.algorithm
        for step in range(int(cfg.total_samples_ncsn // cfg.nr_epochs_ncsn // cfg.batch_size_ncsn)):
            selected = torch.randint(len(self.expert_data["states"]), (cfg.batch_size_ncsn,), device=self.device)
            samples = torch.cat([self.expert_data["states"][selected], self.expert_data["next_states" if cfg.state_based else "actions"][selected]], dim=-1)
            losses = []
            for indices in self.minibatches(cfg.batch_size_ncsn, cfg.minibatch_size_ncsn, cfg.nr_epochs_ncsn):
                conditions = torch.randint(cfg.L_ncsn, (len(indices),), device=self.device)
                loss = self.energy_loss(samples[indices], conditions, torch.randn_like(samples[indices]))
                self.optimize("energyfn", loss)
                losses.append(loss.detach())
            self.log({"loss/energyfn_loss": torch.stack(losses).mean()}, (step + 1) * cfg.nr_epochs_ncsn * cfg.batch_size_ncsn)
    ...
    def update(self, batch, learning_iteration_step):
        ...
        metrics = {}
        with torch.no_grad():
            samples = torch.cat([states, next_states if cfg.state_based else actions], dim=-1)
            sigmas = torch.exp(torch.linspace(math.log(cfg.sigma_begin_ncsn), math.log(cfg.sigma_end_ncsn), cfg.L_ncsn, device=self.device))
            corrected_reward = self.energyfn(samples, cfg.sigma_inference_ncsn if cfg.ncsnv1 else sigmas[cfg.sigma_inference_ncsn])
            absorbing_reward = torch.zeros_like(corrected_reward)
        with torch.no_grad():
